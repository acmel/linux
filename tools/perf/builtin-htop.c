...
static void print_sym_table(void)
{
	...
	if (top.sym_filter_entry) {
		show_details(top.he_filter_entry);
		return;
	}
...
{
	char *buf = malloc(0), *p;
	struct hist_entry *he = *target, *n, *found = NULL;
	struct rb_node *next;
	size_t dummy = 0;

	/* zero counters of active symbol */
	if (he) {
		__zero_source_counters(he);
		*target = NULL;
	}
...
static void print_mapped_keys(void)
{
	...
	if (top.sym_filter_entry) {
		struct symbol *sym = top.he_filter_entry->ms.sym;
		name = sym->name;
	}
...
static void handle_keypress(int c)
{
	...
	switch (c) {
		...
		case 'Q':
			...
			exit(0);
		case 's':
			prompt_symbol(&top.he_filter_entry, "Enter details symbol");
			break;
		case 'S':
			if (!top.sym_filter_entry)
				break;
			else {
				struct hist_entry *he = top.he_filter_entry;
